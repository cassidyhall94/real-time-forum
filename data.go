...
func (forum *Forum) CreatePost(post PostFeed) error {
	stmt, err := forum.DB.Prepare("INSERT INTO post (username, title, content, category, dateCreated, image) VALUES (?, ?, ?, ?, ?, ?);")
	defer stmt.Close()
	if err != nil {
		return fmt.Errorf("CreatePost DB Prepare error: %+v\n", err)
	}
	_, err = stmt.Exec(post.Username, post.Title, post.Content, post.Category, post.CreatedAt, post.Image)
	if err != nil {
...
func (data *Forum) GetPosts() ([]PostFeed, error) {
	...
	for rows.Next() {
		err := rows.Scan(&id, &uiD, &title, &content, &category, &created, &image)
		if err != nil {
